...
def save_q_dict(q_dict):
    ...
    try:
        ...
        print("Q-table successfully saved to q_table.pkl.")
...
def get_light_state():
    ...
    print("IR SENSOR", ir_sensor.distance())
...
def turn_left(robot, previous_light_state):
    ...
    ev3.screen.draw_text(30, 40, "Group 1 :)")
    # ev3.screen.load_image("/home/robot/mylegoimage.bmp")
...
def turn_right(robot, previous_light_state):
    # ev3.speaker.play_file(SoundFile.TURN)
    ev3.screen.draw_text(30, 40, "Group 1 :D")
    while previous_light_state == get_light_state():
        robot.drive(10, 110)
        wait(100)
...
m_y = [
    ("MIDDLE", "turn_right", "BLACK"),
    ("BLACK", "turn_left", "MIDDLE"),
    ("MIDDLE", "turn_left", "WHITE"),
    ("WHITE", "turn_right", "MIDDLE"),
]
m_x = [
    ("MIDDLE", "turn_right", "WHITE"),
    ("WHITE", "turn_left", "MIDDLE"),
    ("MIDDLE", "turn_left", "BLACK"),
    ("BLACK", "turn_right", "MIDDLE"),
]
...
def learn():
    ...
    while True:
        ...
        if random.uniform(0, 1) < E ** (iterations / -TEMP):
            ...
            action = get_best_action(Q_table, mode, light_state)[
                0
        ...
        action(
            robot, light_state
        )  # Execute the action and wait for the robot to finish
        ...
        # Update Q-table
        Q_table[(mode, light_state, action)] += ALPHA * (
            reward_next + GAMMA * max_q_next - Q_table[(mode, light_state, action)]
        )
